...
const createSprite = (parent = new UICanvas(), spriteDefinition) => {
    const image = new UIImage(parent, new Texture(getImageSrc(`sprite01`), {hasAlpha:true, samplingMode:0}));
...
export type TabDefinition = {
    id:string,
    label?:string,
    spriteDefinition:SpriteDefiniton
}
type TabOptions = {
...
const createTab = (parent = new UICanvas(), options:TabOptions) => {
    ...
    const handlerImg = new UIImage(handler, new Texture(getImageSrc(`sprite1`)));
